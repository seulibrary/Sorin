defmodule ApiWeb.V1.SearchController do
  ...
  def search(conn, params) do    
    ...
    {status, user_id} = Phoenix.Token.verify(conn, "user_id", conn.private[:plug_session]["user_id"], max_age: 86400)

    # if type param is being passed in, it's an append (not the item_type under filters)
    if params["type"] do
      case params["type"] do
        "catalog" ->
          Logger.info"> Search: #{params["query"]}, limit: #{limit}, offset: #{offset}, filters: #{params["filters"] }, type: catalog"

          conn
          |> put_view(ApiWeb.SearchView)
          |> remove_non_authorized_content(user_id, "catalogs.json", %{results: Search.search_catalogs(params["query"], limit, offset, filters)})

        "user" ->
          Logger.info"> Search: #{params["query"]}, limit: #{limit}, offset: #{offset}, filters: #{params["filters"] }, type: user"

          conn
          |> put_view(ApiWeb.SearchView)
          |> remove_non_authorized_content(user_id, "users.json", %{results: Search.search_users(params["query"], limit, offset)})
          
        "collection" ->
          Logger.info"> Search: #{params["query"]}, limit: #{limit}, offset: #{offset}, filters: #{params["filters"] }, type: collection"

          conn
          |> put_view(ApiWeb.SearchView)
          |> remove_non_authorized_content(user_id, "collections.json", %{results: Search.search_collections(params["query"], limit, offset)})
      end
    else
      case Search.all(params["query"], limit, offset, filters) do
        {:ok, search} ->
          Logger.info"> Search: #{params["query"]}, limit: #{limit}, offset: #{offset}, filters: #{params["filters"] }, type: all"

          conn
          |> put_view(ApiWeb.SearchView)
          |> remove_non_authorized_content(user_id, "index.json", %{search: search})
        {:error, _} ->
          Logger.error"> Search Error: #{params["query"]}, limit: #{limit}, offset: #{offset}, filters: #{params["filters"] }, type: all"
  ...
  defp remove_non_authorized_content(conn, :missing, "catalogs.json", %{:results => _content} = data) do
    conn
